...
void PMXFile::Read(std::istream& is, void* dst, const std::size_t bytes) {
	is.read(static_cast<char*>(dst), bytes);
}

std::streampos PMXFile::GetFileEnd(std::istream& is) {
...
bool PMXFile::HasMore(std::istream& is, const std::streampos& end) {

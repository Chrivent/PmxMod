...
namespace Chrivent {
	Joint::Joint(const JointDefinition& definition, const RigidBody& rigidBodyA, const RigidBody& rigidBodyB) {
		...
		btTransform invA = rigidBodyA.ResolveRigidBody().getWorldTransform().inverse();
		btTransform invB = rigidBodyB.ResolveRigidBody().getWorldTransform().inverse();
		invA = invA * transform;
		invB = invB * transform;
		auto jointConstraint = std::make_unique<btGeneric6DofSpringConstraint>(
			rigidBodyA.ResolveRigidBody(), rigidBodyB.ResolveRigidBody(),
			invA, invB, true);

...
namespace Chrivent {
	...
	public:
		...
		static bool Read(std::istream& is, void* dst, std::size_t bytes);
		// PMX 헤더의 인덱스 크기 규칙에 맞춰 가변 크기 인덱스를 읽는다.
		static bool ReadIndex(std::istream& is, int32_t* index, uint8_t indexSize);
		// POD 값을 바이너리 스트림에서 읽는다.
		template <typename T>
		static bool Read(std::istream& is, T* dst) {
			return Read(is, dst, sizeof(T));
		}
	};
}
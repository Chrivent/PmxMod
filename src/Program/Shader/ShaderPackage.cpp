...
namespace Chrivent {
	...
	ShaderPackageDiscovery ShaderPackageLoader::Discover(const std::filesystem::path& packagesDirectory) {
		...
		for (const std::filesystem::path& manifestPath : manifestPaths) {
			...
			if (!packageIds.emplace(package.id).second) {
				discovery.errors.emplace_back(ShaderPackageError{
					...
					.message = "중복된 셰이더 패키지 id입니다: " + package.id
				});
				continue;
			}
		...
		if (error)
			discovery.errors.emplace_back(ShaderPackageError{
				...
				.message = "셰이더 패키지 디렉터리를 검색하지 못했습니다: " + packagesDirectory.string()
			});
	...
	bool ShaderPackageParser::ReadJsonObject(
		...
		if (!stream) {
			error = "JSON 파일을 열지 못했습니다: " + path.string();
			return false;
		}
		json = nlohmann::json::parse(stream, nullptr, false);
		if (json.is_object())
			return true;
		error = "JSON 최상위 값이 객체가 아닙니다: " + path.string();
		return false;
	}
	...
	bool ShaderPackageParser::ReadRequiredString(
		...
		error = "필수 문자열 필드가 없거나 올바르지 않습니다: " + std::string(key);
		return false;
	}
	...
	bool ShaderPackageParser::ResolvePackagePath(const std::filesystem::path& packageRoot,
		...
		if (std::filesystem::is_regular_file(resolvedPath, filesystemError) && !filesystemError)
			return true;
		error = "패키지 파일이 없습니다: " + resolvedPath.string();
		return false;
	}
	...
	bool ShaderPackageParser::LoadPass(const std::filesystem::path& packageRoot,
		...
		if (!json.is_object()) {
			error = "효과 pass 정의가 올바르지 않습니다: " + manifestPath.string();
			return false;
		}
		...
		for (const auto& inputJson : *reads) {
			...
			if (input.slot >= PostProcessInputLayout::maxTextureCount || !usedSlots.emplace(input.slot).second
				...
				error = "후처리 입력 slot이 올바르지 않거나 중복되었습니다: " + manifestPath.string();
				return false;
			}
	...
	bool ShaderPackageParser::LoadResources(const nlohmann::json& json,
		...
		if (!resourceArray->is_array()) {
			error = "효과 resources는 배열이어야 합니다: " + manifestPath.string();
			return false;
		}
		...
		for (const auto& resourceJson : *resourceArray) {
			...
			if (!resourceJson.is_object()
				...
				error = "효과 resource가 올바르지 않거나 중복되었습니다: " + manifestPath.string();
				return false;
			}
			if (lifetime == "transient")
				...
				error = "지원하지 않는 효과 resource lifetime입니다: " + lifetime + " / " + manifestPath.string();
				return false;
			}
			if (format == "rgba8_unorm")
				...
				error = "지원하지 않는 효과 resource format입니다: " + format + " / " + manifestPath.string();
				return false;
			}
			...
			if (size != resourceJson.end() && resolution != resourceJson.end()) {
				error = "효과 resource에 size와 resolution을 함께 선언할 수 없습니다: " + manifestPath.string();
				return false;
			}
			if (size != resourceJson.end()) {
				if (!size->is_object() || !size->contains("width") || !size->contains("height")
					...
					error = "고정 효과 resource 크기가 올바르지 않습니다: " + manifestPath.string();
					return false;
				}
				...
				if (resource.definition.width == 0 || resource.definition.height == 0) {
					error = "고정 효과 resource 크기는 0일 수 없습니다: " + manifestPath.string();
					return false;
				}
			} else {
				...
				if (value == "full")
					...
					error = "지원하지 않는 효과 resource resolution입니다: " + value + " / " + manifestPath.string();
					return false;
				}
			}
	...
	bool ShaderPackageParser::LoadParameters(const nlohmann::json& json,
		const std::filesystem::path& manifestPath, std::vector<EffectParameterMetadata>& metadata,
		...
		if (!parameterArray->is_array()) {
			error = "효과 parameters는 배열이어야 합니다: " + manifestPath.string();
			return false;
		}
		...
		for (const auto& parameterJson : *parameterArray) {
			if (!parameterJson.is_object()) {
				error = "효과 parameter가 올바르지 않습니다: " + manifestPath.string();
				return false;
			}
			EffectParameterMetadata parameter;
			...
			if (parameter.slot >= PostProcessInputLayout::maxParameterCount
				...
				error = "효과 parameter slot이 올바르지 않거나 중복되었습니다: " + manifestPath.string();
				return false;
			}
			values.emplace_back(EffectParameterValue{
				.slot = parameter.slot,
				.value = parameter.defaultValue
			});
			metadata.emplace_back(std::move(parameter));
		}
		return true;
	}
	...
	bool ShaderPackageParser::LoadEffect(const std::filesystem::path& packageRoot,
		...
		if (effectJson.value("schemaVersion", 0) != schemaVersion) {
			error = "지원하지 않는 효과 schemaVersion입니다: " + manifestPath.string();
			return false;
		}
		...
		auto& [parameterValues, resources, passes] = effect.runtime;
		std::string typeName;
		...
		for (const auto& inputJson : *inputValues) {
			...
			if ((inputName != "scene_color" && inputName != "scene_depth" && inputName != "scene_velocity")
				...
				error = "engine input이 지원되지 않거나 중복되었습니다: " + inputName + " / " + manifestPath.string();
				return false;
			}
		}
		std::vector<ParsedEffectResource> parsedResources;
		if (!LoadParameters(effectJson, manifestPath, effect.parameters, parameterValues, error)
		...
		if (passValues == effectJson.end() || !passValues->is_array() || passValues->empty()) {
			error = "효과에는 하나 이상의 pass가 필요합니다: " + manifestPath.string();
			return false;
		}
		...
		for (const auto& passJson : *passValues) {
			...
			if (!uniquePassNames.emplace(pass.name).second) {
				error = "효과 pass 이름이 중복되었습니다: " + pass.name + " / " + manifestPath.string();
				return false;
			}
		...
		for (auto& resource : parsedResources) {
			if (resource.name == "effect_input" || resource.name == "effect_output"
				...
				error = "효과 resource가 예약된 이름을 사용합니다: "
		...
		for (size_t passIndex = 0; passIndex < parsedPasses.size(); passIndex++) {
			...
			for (const auto& [slot, resource] : parsedPass.inputs) {
				...
				if (resource == "scene_color" || resource == "scene_depth" || resource == "scene_velocity") {
					if (!uniqueInputNames.contains(resource)) {
						error = "pass가 선언되지 않은 engine input을 사용합니다: "
							+ resource + " / " + manifestPath.string();
						return false;
					}
				...
				if (resourceDefinition == resourceDefinitions.end()
					...
					error = "pass 입력 resource가 올바르지 않거나 초기화되지 않았습니다: "
						+ resource + " / " + manifestPath.string();
					return false;
				}
			...
			if (parsedPass.output == "effect_output") {
				if (passIndex + 1 != parsedPasses.size()) {
					error = "마지막 효과 pass만 effect_output에 쓸 수 있습니다: " + manifestPath.string();
					return false;
				}
			...
			if (outputResourceDefinition == resourceDefinitions.end()
				...
				error = "pass 출력 resource가 올바르지 않습니다: "
					+ parsedPass.output + " / " + manifestPath.string();
				return false;
			}
			if (!writesHistoryResource && std::ranges::any_of(parsedPass.inputs,
				...
				error = "한 pass에서 같은 transient resource를 읽고 쓸 수 없습니다: "
	...
	bool ShaderPackageParser::LoadPackage(const std::filesystem::path& manifestPath,
		...
		if (json.value("schemaVersion", 0) != schemaVersion) {
			error = "지원하지 않는 패키지 schemaVersion입니다: " + manifestPath.string();
			return false;
		}
		...
		for (const auto& effectPathJson : *effectArray) {
			if (!effectPathJson.is_string()) {
				error = "패키지 효과 경로는 문자열이어야 합니다: " + manifestPath.string();
				return false;
			}

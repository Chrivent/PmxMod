...
namespace Chrivent {
	...
	public:
		...
		bool Initialize(const Dx12Device& sourceDevice, std::span<const EffectPassDefinition> passes,
			std::span<const DXGI_FORMAT> formats);
		ID3D12RootSignature* ResolveRootSignature() const { return rootSignature.Get(); }
		ID3D12PipelineState* ResolvePipelineState(const size_t index) const {

...
namespace Chrivent {
	...
	public:
		...
		Dx12Texture Load(const Dx12DeviceInfo& deviceInfo, const std::filesystem::path& texturePath);
		// 텍스처가 없는 material에 바인딩할 흰색 DX12 텍스처를 생성한다.
		Dx12Texture CreateWhiteTexture(const Dx12DeviceInfo& deviceInfo);
	};
}

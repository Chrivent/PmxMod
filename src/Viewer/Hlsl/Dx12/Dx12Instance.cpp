...
namespace Chrivent {
	...
	bool Dx12Instance::CreateConstantBuffers(const Dx12Device& device) {
		const size_t vertexConstantSize = Dx12Buffer::AlignConstantBufferSize(sizeof(ShaderModelVertexConstants));
		...
		const size_t edgePixelConstantSize = Dx12Buffer::AlignConstantBufferSize(sizeof(ShaderEdgePixelConstants));
		const size_t groundShadowVertexConstantSize = Dx12Buffer::AlignConstantBufferSize(sizeof(ShaderGroundShadowVertexConstants));
		const size_t groundShadowPixelConstantSize = Dx12Buffer::AlignConstantBufferSize(sizeof(ShaderGroundShadowPixelConstants));
		for (size_t frameIndex = 0; frameIndex < kBufferedFrames; frameIndex++) {
			if (!modelVertexConstantBuffers[frameIndex].InitializeUpload(device, vertexConstantSize) ||
				!edgeVertexConstantBuffers[frameIndex].InitializeUpload(device, edgeVertexConstantSize) ||
		...
		edgeSizeConstantBuffers.resize(materialCount);
		for (auto& buffers : edgeSizeConstantBuffers) {
			buffers = std::make_unique<Dx12Buffer[]>(kBufferedFrames);
			for (size_t i = 0; i < kBufferedFrames; i++) {
				if (!buffers[i].InitializeUpload(device, edgeSizeConstantSize))
					return false;
			}
		}
	...
	void Dx12Instance::Clear() {
		...
		for (Dx12Buffer& buffer : edgeVertexConstantBuffers)
			buffer.Destroy();
		for (auto& buffers : edgeSizeConstantBuffers) {
			if (!buffers)
				continue;

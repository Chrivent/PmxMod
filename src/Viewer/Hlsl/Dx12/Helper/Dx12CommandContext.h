...
namespace Chrivent {
	...
	public:
		...
		bool Initialize(const Dx12Device& deviceInfo);
		// 한 프레임의 명령 기록을 시작할 수 있도록 allocator와 list를 초기화한다.
		bool BeginFrame(const Dx12Device& deviceInfo, UINT frameIndex);
		// 기록한 명령 리스트를 닫고 command queue에 제출한다.
		bool Execute(const Dx12Device& deviceInfo);
		// CPU와 GPU를 동기화해 기록된 명령이 모두 끝날 때까지 대기한다.
		bool WaitForGpu(const Dx12Device& deviceInfo);

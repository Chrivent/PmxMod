...
namespace Chrivent {
	...
	public:
		...
		void DrawIndexed(uint32_t imageIndex, const VulkanBufferInfo& vertexBuffer, const VulkanBufferInfo& indexBuffer, VkIndexType indexType, uint32_t firstIndex, uint32_t indexCount) const;
		// 현재 command buffer에 graphics descriptor set들을 바인딩한다.
		void BindDescriptorSets(uint32_t imageIndex, VkPipelineLayout pipelineLayout, uint32_t firstSet, const VkDescriptorSet* descriptorSets, uint32_t descriptorSetCount) const;

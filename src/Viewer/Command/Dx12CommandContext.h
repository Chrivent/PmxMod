...
namespace Chrivent {
	...
	public:
		...
		GraphicsResult<void> BeginFrame(const Dx12Device& sourceDevice, UINT frameIndex);
		// 기록한 명령 리스트를 닫고 command queue에 제출한다.
		GraphicsResult<void> Execute(const Dx12Device& sourceDevice);
		// CPU와 GPU를 동기화해 기록된 명령이 모두 끝날 때까지 대기한다.
		GraphicsResult<void> WaitForGpu(const Dx12Device& sourceDevice);

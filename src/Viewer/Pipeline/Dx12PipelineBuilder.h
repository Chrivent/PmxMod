...
namespace Chrivent {
	...
	public:
		...
		static GraphicsError::Result<void> CreateRootSignature(const Dx12Device& sourceDevice,
		...
		static GraphicsError::Result<std::vector<uint8_t>> CompileShader(
			const Dx12Device& sourceDevice, const std::filesystem::path& file,
			const std::string& entry, bool vertexShader);
		// 셰이더를 컴파일해 완성된 description으로 graphics pipeline state를 생성한다.
		static GraphicsError::Result<void> CreateGraphicsPipelineState(const Dx12Device& sourceDevice,

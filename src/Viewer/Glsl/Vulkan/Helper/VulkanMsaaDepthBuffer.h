...
namespace Chrivent {
	class VulkanMsaaDepthBuffer {
		...
		static VkFormat FindDepthFormat(const VulkanDevice& deviceInfo);
		// 후보 format 중 tiling과 feature 조건을 만족하는 format을 찾는다.
		static VkFormat FindSupportedFormat(const VulkanDevice& deviceInfo, const VkFormat* candidates, uint32_t candidateCount, VkImageTiling tiling, VkFormatFeatureFlags features);
		// 멀티샘플 depth attachment로 사용할 image와 메모리를 생성한다.
		bool CreateImage(const VulkanDevice& deviceInfo, const VulkanSwapChain& swapChainInfo);

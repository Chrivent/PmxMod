...
namespace Chrivent {
	...
	class VulkanTextureCache : public TextureCache {
		...
		static bool BeginSingleTimeCommands(const VulkanDevice& deviceInfo, VkCommandPool commandPool, VkCommandBuffer& commandBuffer);
		// 일회성 command buffer를 제출하고 해제한다.
		static bool EndSingleTimeCommands(const VulkanDevice& deviceInfo, VkCommandPool commandPool, VkCommandBuffer commandBuffer);
		...
		bool UploadRgbaPixels(const VulkanDevice& deviceInfo, VkCommandPool commandPool, const unsigned char* pixels, uint32_t width, uint32_t height, VulkanTexture& texture, bool clamp) const;
		// Vulkan image와 전용 메모리를 생성한다.
		static bool CreateImage(const VulkanDevice& deviceInfo, uint32_t width, uint32_t height, VkImage& image, VkDeviceMemory& imageMemory);
	...
	public:
		...
		VulkanTexture Load(const VulkanDevice& deviceInfo, VkCommandPool commandPool, const std::filesystem::path& texturePath, bool clamp = false);
		// 텍스처가 없는 재질에 사용할 1x1 흰색 Vulkan 텍스처를 생성한다.
		VulkanTexture CreateWhiteTexture(const VulkanDevice& deviceInfo, VkCommandPool commandPool);
	};
}

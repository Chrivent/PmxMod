...
namespace Chrivent {
	...
	bool VulkanInstance::SetupConstantRings(const VulkanDevice& device) {
		...
		std::string error;
		if (!modelVertexConstantsRing.Setup(device, AlignedSize(sizeof(ShaderModelVertexConstants)) * ringSlack * bufferedFrames, error))
			return false;
		if (!edgeVertexConstantsRing.Setup(device, AlignedSize(sizeof(GlslEdgeVertexConstants)) * (drawCount + ringSlack) * bufferedFrames, error))
			return false;
		...
		if (!edgePixelConstantsRing.Setup(device, AlignedSize(sizeof(ShaderEdgePixelConstants)) * (drawCount + ringSlack) * bufferedFrames, error))
			return false;
		if (!groundShadowPixelConstantsRing.Setup(device, AlignedSize(sizeof(ShaderGroundShadowPixelConstants)) * (drawCount + ringSlack) * bufferedFrames, error))
			return false;
		return true;
	}

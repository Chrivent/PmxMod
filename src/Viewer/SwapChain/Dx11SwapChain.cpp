...
namespace Chrivent {
	GraphicsResult<void> Dx11SwapChain::Initialize(ID3D11Device* device, HWND__* window) {
		Reset();
	...
	GraphicsResult<void> Dx11SwapChain::Resize(const int width, const int height) const {
	...
	GraphicsResult<void> Dx11SwapChain::Present() const {
		if (!swapChain) {
			return std::unexpected(MakeGraphicsError(GraphicsApi::DirectX11,

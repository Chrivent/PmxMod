...
namespace Chrivent {
	...
	class Dx12SwapChain {
		...
		GraphicsError::Result<void> CreateRenderTargetViews(const Dx12Device& sourceDevice);
	...
	public:
		...
		GraphicsError::Result<void> Initialize(const Dx12Device& sourceDevice, HWND hwnd,
			int width, int height);
		// 창 크기에 맞춰 스왑체인을 다시 생성한다.
		GraphicsError::Result<void> Resize(const Dx12Device& sourceDevice, int width, int height);
		// swap chain의 현재 back buffer를 화면에 표시한다.
		GraphicsError::Result<void> Present();

...
namespace Chrivent {
	...
	class VulkanTextureCache : public TextureCache<VulkanTexture> {
		...
		bool UploadRgbaPixels(const VulkanDevice& sourceDevice, const unsigned char* pixels,
	...
	public:
		...
		VulkanTexture Load(const VulkanDevice& sourceDevice,
			const std::filesystem::path& texturePath, bool clamp = false);
		// 텍스처가 없는 재질에 사용할 1x1 흰색 Vulkan 텍스처를 생성한다.
		VulkanTexture CreateWhiteTexture(const VulkanDevice& sourceDevice);
	};
}

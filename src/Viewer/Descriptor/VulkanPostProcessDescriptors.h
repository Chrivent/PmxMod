...
namespace Chrivent {
	...
	class VulkanPostProcessDescriptors {
		...
		GraphicsError::Result<void> CreateLayouts();
		// 후처리 공통 sampler를 생성한다.
		GraphicsError::Result<void> CreateSampler();
		...
		GraphicsError::Result<void> BindBuffers(std::span<const std::unique_ptr<VulkanBuffer>> frameDataBuffers,
	...
	public:
		...
		GraphicsError::Result<void> Initialize(VkDevice sourceDevice, size_t sourceImageCount, size_t sourcePassCount,

...
namespace Chrivent {
	void Dx11Drawer::BindTexture(
		...
		? texture.textureView.Get() : renderer.GetDummyTexture().textureView.Get();
		ID3D11SamplerState* samplers = texture.texture
		? sampler : renderer.GetPipelineStates().textureSampler.Get();
		if (lastView != views) {
			renderer.GetDeviceResources().context->PSSetShaderResources(slot, 1, &views);
			lastView = views;
		}
		if (lastSampler != samplers) {
			renderer.GetDeviceResources().context->PSSetSamplers(slot, 1, &samplers);
	...
	Dx11Drawer::Dx11Drawer(const Dx11Instance& sourceInstance, Dx11Viewer& sourceViewer)

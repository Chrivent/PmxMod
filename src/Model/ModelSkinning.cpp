...
namespace Chrivent {
	...
	void ModelSkinning::Update() const {
		if (model.geometry.updateRanges.empty() ||
			model.geometry.parallelUpdateCount != model.geometry.updateRanges.size())
			SetupParallelUpdate();
		...
		for (size_t i = 0; i < futureCount; i++) {
			if (model.geometry.updateRanges[i + 1].vertexCount != 0)
				model.geometry.parallelUpdateFutures[i] = std::async(std::launch::async,
					[this, range = model.geometry.updateRanges[i + 1]] { UpdateVertices(range); });
		}
		UpdateVertices(model.geometry.updateRanges[0]);
		for (size_t i = 0; i < futureCount; i++) {
			if (model.geometry.updateRanges[i + 1].vertexCount != 0)

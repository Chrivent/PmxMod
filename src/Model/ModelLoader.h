...
namespace Chrivent {
	...
	class ModelLoader {
		...
		void LoadVertices(const PmxParser& pmx, const glm::vec3& invZ) const;
		// PMX 면 인덱스를 렌더링용 인덱스 버퍼로 변환한다.
		bool LoadFaces(const PmxParser& pmx) const;
		...
		void LoadNodes(const PmxParser& pmx, const glm::vec3& invZ) const;
		// PMX 모프 정보를 타입별 모프 데이터로 변환한다.
		void LoadMorphs(const PmxParser& pmx, const glm::vec3& invZ) const;
		// 순환 참조가 있는 그룹 모프를 끊어 무한 재귀를 막는다.
		void FixInfiniteGroupMorphs() const;
		// PMX 강체와 조인트를 물리 월드에 등록한다.
		void LoadPhysics(const PmxParser& pmx) const;

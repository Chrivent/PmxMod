...
namespace Chrivent {
    ...
    std::string Util::WStringToUtf8(const std::wstring& w) {
        ...
        if (w.empty())
            return utf8;
        const int need = WideCharToMultiByte(CP_UTF8, 0, w.data(), w.size(), nullptr, 0, nullptr, nullptr);
        if (need <= 0)
            return utf8;
        utf8.resize(need);
        const int written = WideCharToMultiByte(CP_UTF8, 0, w.data(), w.size(), utf8.data(), need, nullptr, nullptr);
        if (written != need)
            utf8.clear();
        return utf8;
    }
    ...
    std::string Util::SjisToUtf8(const char* sjis) {
        ...
        if (!sjis)
            return result;
        const int need = MultiByteToWideChar(932, MB_ERR_INVALID_CHARS, sjis, -1, nullptr, 0);
        if (need <= 0)
            return result;
        std::wstring w(need, L'\0');
        const int written = MultiByteToWideChar(932, MB_ERR_INVALID_CHARS, sjis, -1, w.data(), need);
        if (written <= 0)
            return result;
